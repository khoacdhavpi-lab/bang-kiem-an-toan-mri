/**
 * Trình tạo mã QR thuần JavaScript (Không phụ thuộc thư viện ngoài)
 * Dựa trên thuật toán QR Code Type 4-10 chuẩn ISO/IEC 18004
 */
(function(global) {
  // Bảng tạo QR nhỏ gọn & chuẩn xác
  ...
  // Hàm tiện ích tạo SVG hoặc vẽ lên Canvas
  global.QRCode = {
    generateSVG: function(text, size = 200, margin = 2) {
      var type = 4;
      if (text.length > 50) type = 6;
      if (text.length > 90) type = 8;
      if (text.length > 130) type = 10;
      var qr = new QRCodeModel(type, 1); // Level M
      qr.addData(text);
      qr.make();
      ...
      return '<svg xmlns="http://www.w3.org/2000/svg" width="' + size + '" height="' + size + '" viewBox="0 0 ' + viewBoxSize + ' ' + viewBoxSize + '" shape-rendering="crispEdges"><rect width="100%" height="100%" fill="#ffffff"/>' + rects.join('') + '</svg>';
    },

    drawToCanvas: function(canvas, text, size = 240) {
      var svgStr = this.generateSVG(text, size);
      var img = new Image();
